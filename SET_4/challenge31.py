...
def get_next_byte(known_bytes, filename, rounds):
    ...
    times = [[] for _ in range(256)] # each character is tried rounds number of times

    # When life gives you a shitty timer : sample a lot!!
    for _ in range(rounds):
        ...
        for i in range(256):
            ...
            response = requests.get('http://localhost:8000/test?file={}&signature={}'.format(
                filename, signature))

            # Just in case we found the correct signature already, 
    ...
    # Get the index of the item which took the highest median time for
    # the requests: cos the longer it takes the further i got
...
def discover_mac_with_timing_attack(filename, rounds):
    ...
    while len(known_bytes) < HMAC_LEN:
        ...
        # every time i get the best guy from which one goes further
    ...
    response = requests.get('http://localhost:8000/test?file=' + filename + '&signature=' + signature)
